...
{}
return;}};ipb.quickpm.init();;var _idx=window.IPBoard;_idx.prototype.board={_statusClick:0,_statusDefaultValue:'',count_sbars:0,count_hidden:0,init:function()
...
{if($('hookStatusTextField')){$('hookStatusTextField').value='';}
...
{ipb.delegate.register(".category_block .toggle",ipb.board.toggleCat);var cookie=ipb.Cookie.get('toggleCats');if(cookie)
...
{var wrapper=$('category_'+cookies[abcdefg]).down('.table_wrap');wrapper.hide();$('category_'+cookies[abcdefg]).addClassName('collapsed');newCookies.push(cookies[abcdefg]);}
else
{resetCookie=true;}}}
if(resetCookie)
{ipb.Cookie.set('toggleCats',newCookies.join(','));}}},toggleCat:function(e,elem)
{if(ipb.board.animating){return false;}
var remove=$A();var catname=$(elem).up('.category_block');var wrapper=$(catname).down('.table_wrap');$(wrapper).identify();var catid=catname.id.replace('category_','');ipb.board.animating=true;var cookie=ipb.Cookie.get('toggleCats');if(cookie==null){cookie=$A();}else{_cookie=cookie.split(',');cookie=$A();_cookie.each(function(item)
{if(item)
{cookie.push(item);}});}
Effect.toggle(wrapper,'blind',{duration:0.4,afterFinish:function(){ipb.board.animating=false;}});Effect.toggle(wrapper,'appear',{duration:0.3});if(catname.hasClassName('collapsed'))
...
_cookie=cookie;cookie=$A();_cookie.each(function(item)
{ok=true;remove.each(function(ritem)
{if(ritem==item)
{ok=false;}})
if(ok===true)
{cookie.push(item);}});cookie=","+cookie.uniq().join(',')+",";ipb.Cookie.set('toggleCats',cookie,1);Event.stop(e);},initSidebar:function()
{if($('toggle_sidebar'))
{$('toggle_sidebar').on('click',ipb.board.toggleSidebar);if($('index_stats').visible()){$('toggle_sidebar').update($('toggle_sidebar').readAttribute("data-open"));}else{$('toggle_sidebar').update($('toggle_sidebar').readAttribute("data-closed"));}}
ipb.board.setUpSideBarBlocks();},toggleSidebar:function(e){if($('index_stats').visible())
{ipb.Cookie.set('hide_sidebar',1,1);new Effect.Fade($('index_stats'),{duration:0.4,afterFinish:function(){new Effect.Morph($('board_index'),{duration:0.3,style:'no_sidebar'});}});$('toggle_sidebar').update("&laquo;");}
else
{ipb.Cookie.set('hide_sidebar',0,1);new Effect.Morph($('board_index'),{duration:0.3,style:'force_sidebar',afterFinish:function(){new Effect.Appear($('index_stats'),{duration:0.4,afterFinish:function(){$('board_index').removeClassName('no_sidebar').removeClassName('force_sidebar');}});}});$('toggle_sidebar').update("&times;");}
Event.stop(e);},setUpSideBarBlocks:function()
{if($('index_stats')&&$('index_stats').visible())
{$$('#index_stats h3').each(function(h3)
{$(h3).identify();ipb.board.count_sbars++;title=escape($(h3).innerHTML.replace(/<(\/)?([^>]+?)>/g,'')).replace(/%/g,'');title=title.replace(/ /g,'').replace(/[^a-zA-Z0-9]/,'').toLowerCase();$(h3).up('div').addClassName('__xX'+title);$(h3).update('<a href="#" class="ipsSidebar_trigger ipsType_smaller right desc mod_links">'+ipb.lang['hide']+'</a>'+h3.innerHTML);});cookie=ipb.Cookie.get('toggleSBlocks');if(cookie)
...
ipb.board.count_hidden++;top.down('.ipsSidebar_trigger').update(ipb.lang['unhide']);wrapper.hide();}}}}}}
ipb.delegate.register(".ipsSidebar_trigger",ipb.board.toggleSideBarBlock);},toggleSideBarBlock:function(e,elem)
{Event.stop(e);elem.identify();var remove=$A();cookie=ipb.Cookie.get('toggleSBlocks');if(cookie==null){cookie=$A();}else{_cookie=cookie.split(',');cookie=$A();_cookie.each(function(item)
{if(item)
{cookie.push(item);}});}
...
{if($(top).hasClassName('alt'))
{$(top)._isAlt=true;$(top).removeClassName('alt');}
top.down('.ipsSidebar_trigger').update(ipb.lang['unhide']);ipb.board.count_hidden--;Debug.write("Adding "+topId);cookie.push(topId);}
else
{if($(top)._isAlt)
{$(top).addClassName('alt');}
top.down('.ipsSidebar_trigger').update(ipb.lang['hide']);ipb.board.count_hidden--;Debug.write("Removing "+topId);remove.push(topId);}
...
{ok=false;}})
if(ok===true)
{cookie.push(item);}});cookie=","+cookie.uniq().join(',')+",";ipb.Cookie.set('toggleSBlocks',cookie,1);}};ipb.board.init();